...
void __attribute__((noreturn)) call_start_cpu0(void)
{
    ...
    // Call secure boot startup hook
    secboot_bootloader_on_boot_start();
    ...
    // Note: bootloader_utility_load_boot_image() performs signature verification inside try_load_partition()
    // via bootloader_load_image(). On success, it jumps to the app and never returns.
    // If we reach here after this call, it means all partitions failed verification.
    bootloader_utility_load_boot_image(&bs, boot_index);

    // If we get here, all partitions failed signature verification
    ESP_LOGW(TAG, "All partitions failed signature verification");
    secboot_bootloader_on_signature_verification(false);
}
